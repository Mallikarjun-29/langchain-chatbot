...
def reset_article_state():
    """Clear article context and memory."""
...
def fetch_article(url: str):
    """Fetch article content. Returns (content, error_message)."""
    try:
...
def chat_with_article(article_content: str, question: str, title: str) -> str:
    """Send article + question to Ollama. Returns response string."""
    history = st.session_state.article_memory.load_memory_variables({})
    chat_history = history.get("article_history", "")

    response = st.session_state.ollama_llm.invoke(
...
with st.sidebar:
    ...
    groq_api_key = st.text_input(
        "Groq API Key (for stock queries):",
        type="password",
        help="Free at https://console.groq.com/keys"
    )
    st.caption("💡 Article chat uses free local Ollama")
...
# ===== LLM SETUP =====
# Cache LLMs in session state — avoid reinitializing every rerun
if "ollama_llm" not in st.session_state:
    st.session_state.ollama_llm = ChatOllama(model="llama3.1:8b", temperature=0)
...
def calculator(expression: str) -> str:
    """Use this tool when the user asks to perform mathematical calculations
    such as addition (+), subtraction (-), multiplication (*), division (/).
    Input should be a mathematical expression like '2 + 2'.
...
# ===== AGENT — cached in session state to avoid hub.pull() every rerun =====
if "agent_executor" not in st.session_state or (
    groq_llm and st.session_state.get("last_groq_key") != groq_api_key
):
    if "react_prompt" not in st.session_state:
        st.session_state.react_prompt = hub.pull("hwchase17/react")

    agent_llm = groq_llm if groq_llm else st.session_state.ollama_llm
    agent = create_react_agent(llm=agent_llm, tools=tools, prompt=st.session_state.react_prompt)
...
if not groq_api_key:
    st.warning("⚠️ Add Groq API key in sidebar for stock queries. Article chat works without it!")
...
if prompt_input := st.chat_input("Ask about stocks, paste a URL, or type article number..."):
    ...
    with st.chat_message("assistant"):
        ...
        # ── NUMBER → load saved article ────────────────────────────────
            ...
            with st.spinner("📰 Loading article..."):
                ...
                if index < 0 or index >= len(st.session_state.article_store):
                    ...
                    # Reset memory only when switching to a different article
                    ...
                    if error:
                        ...
                        output = chat_with_article(content, prompt_input, title)
                        st.info(f"📰 **{title}**")
                        st.markdown(output)
        ...
        # ── FOLLOW-UP on loaded article ────────────────────────────────
            ...
            with st.spinner("💭 Reading article..."):
                ...
                if error:
                    ...
                    output = chat_with_article(
                        content, prompt_input, st.session_state.current_article_title
                    )
        ...
        # ── AGENT question ─────────────────────────────────────────────
            ...
            if not groq_api_key:
                output = "⚠️ Add your Groq API key in the sidebar to ask stock questions!"
                st.markdown(output)
            else:
